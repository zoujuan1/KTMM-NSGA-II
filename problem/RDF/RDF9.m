classdef RDF9 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            Nt = 1 + floor(10*abs(G));
            Nt = round(Nt*1e6)/1e6;
            %temp = cos(4*t + PopDec(:,1))
            g =  1 + sum(( PopDec(:,2:end) - cos(4*t + PopDec(:,1) + PopDec(:,1:end-1))).^2, 2);
            PopObj(:,1) = g.*(PopDec(:,1) + max(0, (0.1 + 0.5/Nt)*sin(2*Nt*pi*PopDec(:,1))));
            PopObj(:,2) = g.*(1-PopDec(:,1) + max(0, (0.1 + 0.5/Nt)*sin(2*Nt*pi*PopDec(:,1))));
        end
        ...
        function R = GetOptimum(obj,N)
            ...
            G = sin(0.5*pi*t);
            G = round(G*1e6)/1e6;
            N_t = 1 + floor(10*abs(G));
            N_t = round(N_t*1e6)/1e6;
           
            x = linspace(0,1,N)';
            obj.Optimums = {};
            for i = 1 : length(G)
                f1 = x + max(0, (0.1 + 0.5/N_t(i))*sin(2*N_t(i)*pi*x));
                f2 = 1 - x + max(0, (0.1 + 0.5/N_t(i))*sin(2*N_t(i)*pi*x));
                R(:,1) = f1;
                R(:,2) = f2;
                R      = R(NDSort(R,1)==1,:);
