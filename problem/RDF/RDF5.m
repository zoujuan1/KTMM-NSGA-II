classdef RDF5 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj(:,1) = g.*(PopDec(:,1) + 0.02*sin(w*pi*PopDec(:,1)));
            PopObj(:,2) = g.*(1-PopDec(:,1) + 0.02*sin(w*pi*PopDec(:,1)));
        end
        ...
        function R = GetOptimum(obj,N)
           % t = floor(0:obj.maxFE/obj.N/obj.taut)/obj.nt;
            ...
            obj.Optimums = {};
            for i = 1 : length(G)
                f1 = x + 0.02*sin(w(i)*pi*x);
                f2 = 1 - x + 0.02*sin(w(i)*pi*x);
                R(:,1) = f1;
                R(:,2) = f2;
                R      = R(NDSort(R,1)==1,:);
