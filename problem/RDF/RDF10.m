classdef RDF10 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            H = round(H*1e6)/1e6;
            temp = sin(2*pi*(PopDec(:,1)+PopDec(:,2)))/(1+abs(G));
            g =  1 + sum(( PopDec(:,3:end)  - temp).^2, 2);
            PopObj(:,1) = g.*sin(0.5*pi*PopDec(:,1)).^H;
            PopObj(:,2) = g.*(sin(0.5*pi*PopDec(:,2)).*cos(0.5*pi*PopDec(:,1))).^H;
            PopObj(:,3) = g.*(cos(0.5*pi*PopDec(:,2)).*cos(0.5*pi*PopDec(:,1))).^H;
        end
        ...
        function R = GetOptimum(obj,N)
            ...
            obj.Optimums = {};
            for i = 1 : length(G)
                f1 = sin(0.5*pi*x1).^H(i);
                f2 = sin(0.5*pi*x2).^H(i).*cos(0.5*pi*x1).^H(i);
                f3 = cos(0.5*pi*x2).^H(i).*cos(0.5*pi*x1).^H(i);
