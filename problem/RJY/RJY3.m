    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
           % t = floor(obj.FE/obj.N/obj.taut)/obj.nt;
            ...
            g = sum((PopDec(:,2:end).^2 - PopDec(:,1:end-1)).^2,2);
            
            PopObj(:,1) = (1+g).*( y(:,1) + A*sin(W*pi.*y(:,1)));
            PopObj(:,2) = (1+g).*(1-y(:,1)+ A*sin(W*pi.*y(:,1)));
        end
        ...
        function R = GetOptimum(obj,N)
            ...
            for i = 1 : length(H)
                t = tt(i);
                W=floor(6*sin(0.5*pi*(t-1)));
                R(:,1) = x + 0.05*sin(W*pi*x);
                R(:,2) = 1 - x + 0.05*sin(W*pi*x);
