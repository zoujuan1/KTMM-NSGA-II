...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj(:,1) =real(g.*( PopDec(:,1) + A*sin(W*pi.*PopDec(:,1))).^a);
            PopObj(:,2) =real(g.*(1-PopDec(:,1)+A*sin(W*pi.*PopDec(:,1))).^b);
        end
        ...
        function R = GetOptimum(obj,N)
            ...
            for i = 1 : length(H)
                ...
                a=2/b;
                R=[];
                R(:,1) = real((x+0.05*sin(6*pi*x)).^a);
                R(:,2) = real((1-x+0.05*sin(6*pi*x)).^b);
